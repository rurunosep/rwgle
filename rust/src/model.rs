...
impl Model {
  pub async fn new(gl: &GL, attributes: &HashMap<String, Attribute>, gltf_url: &str) -> Model {
    let gltf = fetch_resource_as_array_buffer(gltf_url).await;
    ...
    let buffer = if let gltf::buffer::Source::Uri(uri) = gltf.buffers().next().unwrap().source() {
      fetch_resource_as_array_buffer(uri).await
    } else {
      panic!("Can only read gltf buffer from url currently")
    };

    //web_sys::console::log_1(&format!("{}", view.offset()).into());
